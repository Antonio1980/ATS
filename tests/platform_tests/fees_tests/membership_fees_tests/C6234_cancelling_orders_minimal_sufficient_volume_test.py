...
class TestCancelOrderMinimalSufficientVolume(object):

    @allure.step("test_cancel_order_minimal_sufficient")
    ...
    def test_cancel_order_minimal_sufficient(self, r_customer_sql, preconditions, safe_price):
        quantity1 = round((100 / preconditions), 4)
        order1 = r_customer_sql.postman.order_service.create_order_sync(Order().set_order(2, instrument_id, quantity1,
                                                                                      safe_price))

        quantity2 = round((600 / preconditions), 4)
        order2 = r_customer_sql.postman.order_service.create_order_sync(Order().set_order(2, instrument_id, quantity2,
                                                                                      safe_price))

        quantity3 = round((1000 / preconditions), 4)
        order3 = r_customer_sql.postman.order_service.create_order_sync(Order().set_order(2, instrument_id, quantity3,
                                                                                      safe_price))
        ...
        assert results[2] == 2, "Order status isn't changed"
