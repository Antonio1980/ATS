...
@allure.description("""
    Verify that customer able to withdrawal his balance using Withdrawal Wire.
    1) Withdrawal Wire with approved customer
    2)Check balance before and after withdrawal.
    """)
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case, name='Withdrawal Fiat')
@allure.testcase(BaseConfig.GITLAB_URL + "/payment_sanity_tests/fiat_withdrawal_wire_test.py",
                 "TestWithdrawalWireFiat")
...
class TestWithdrawalWireFiat(object):
    ...
    def test_withdrawal_wire_with_approved_customer(self, r_customer):
        logger.logger.info("method test_withdrawal_wire_approved")
        ...
        withdrawal_response = r_customer.postman.payment_service.withdrawal_wire(r_customer.bank, self.currency_id,
