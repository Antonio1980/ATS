...
@allure.feature("Sell order is 50% filled - customer's balance update verified.")
@allure.story("Half filled 'Sell' orders - customer's balance is updated.")
...
@allure.testcase(BaseConfig.GITLAB_URL + "/partially_filled_orders_tests/C6866_filled_sell_50_balance_update_test.py",
                 "TestSellBalanceUpdated_50")
@pytest.mark.usefixtures("r_customer")
class TestSellHalfFilled(object):
    ...
    def test_create_customers(self, r_customer):
        ...
        TestSellHalfFilled.buyer = test_customers[0][0]
        TestSellHalfFilled.seller = test_customers[1][0]

        TestSellHalfFilled.buyer_token = test_customers[0][1]
        TestSellHalfFilled.seller_token = test_customers[1][1]

    @allure.step("Add balance for both customers used for this test.")
    @automation_logger(logger)
    def test_add_balance(self):
        print(F"Test {TestSellHalfFilled.buyer}")
        balance_response_buyer = TestSellHalfFilled.buyer.postman.get_static_postman(TestSellHalfFilled.buyer_token). \
            balance_service.add_balance(TestSellHalfFilled.buyer.customer_id, quoted_currency, 10000)
        ...
        balance_response_seller = TestSellHalfFilled.seller.postman.get_static_postman(TestSellHalfFilled.seller_token). \
            balance_service.add_balance(TestSellHalfFilled.seller.customer_id, base_currency, 10000)
    ...
    def test_change_reference_price(self, r_customer):
        TestSellHalfFilled.original_last_trade_price = Instruments.get_price_last_trade(instrument_id)
        ...
        TestSellHalfFilled.original_ticker_last = Instruments.get_ticker_last_price(instrument_id)
    ...
    def test_place_sell_order(self):
        order_response = TestSellHalfFilled.seller.postman.get_static_postman(TestSellHalfFilled.seller_token). \
            order_service.create_order_sync(TestSellHalfFilled.order_sell)
        assert order_response['error'] is None

        seller_balance = TestSellHalfFilled.seller.postman.get_static_postman(TestSellHalfFilled.seller_token). \
            balance_service.get_all_currencies_balance(TestSellHalfFilled.seller.customer_id)
    ...
    def test_place_buy_order(self):
        order_response = TestSellHalfFilled.buyer.postman.get_static_postman(TestSellHalfFilled.buyer_token). \
            order_service.create_order_sync(TestSellHalfFilled.order_buy)
        assert order_response['error'] is None
    ...
    def test_verify_buyer_balance(self):
        ...
        buyer_balance = TestSellHalfFilled.buyer.postman.get_static_postman(TestSellHalfFilled.buyer_token). \
            balance_service.get_all_currencies_balance(TestSellHalfFilled.buyer.customer_id)
    ...
    def test_restore_original_price(self):
        Instruments.set_price_last_trade(instrument_id, TestSellHalfFilled.original_last_trade_price)
        Instruments.set_ticker_last_price(instrument_id, TestSellHalfFilled.original_ticker_last)
