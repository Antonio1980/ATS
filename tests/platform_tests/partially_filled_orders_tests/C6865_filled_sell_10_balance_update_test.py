...
quantity_buy = quantity_sell / 10  # For this test "quantity_buy" must be 10% of "quantity_sell".

WAIT_FOR_TRADE_DELAY = 4

@allure.feature("Sell order is 10% filled - customer's balance update verified.")
@allure.story("Partially filled 'Sell' orders - customer's balance is updated.")
...
@allure.testcase(BaseConfig.GITLAB_URL + "/partially_filled_orders_tests/C6865_filled_sell_10_balance_update_test.py",
                 "TestSellBalanceUpdated_10")
@pytest.mark.usefixtures("r_customer")
class TestTenSellFilled(object):
    ...
    def test_create_customers(self, r_customer):
        ...
        TestTenSellFilled.buyer = test_customers[0][0]
        TestTenSellFilled.seller = test_customers[1][0]

        TestTenSellFilled.buyer_token = test_customers[0][1]
        TestTenSellFilled.seller_token = test_customers[1][1]

    @allure.step("Add balance for both customers used for this test.")
    @automation_logger(logger)
    def test_add_balance(self):
        print(F"Test {TestTenSellFilled.buyer}")
        balance_response_buyer = TestTenSellFilled.buyer.postman.get_static_postman(TestTenSellFilled.buyer_token). \
            balance_service.add_balance(TestTenSellFilled.buyer.customer_id, quoted_currency, 10000)
        ...
        balance_response_seller = TestTenSellFilled.seller.postman.get_static_postman(TestTenSellFilled.seller_token). \
            balance_service.add_balance(TestTenSellFilled.seller.customer_id, base_currency, 10000)
    ...
    def test_change_reference_price(self, r_customer):
        TestTenSellFilled.original_last_trade_price = Instruments.get_price_last_trade(instrument_id)
        ...
        TestTenSellFilled.original_ticker_last = Instruments.get_ticker_last_price(instrument_id)
    ...
    def test_place_sell_order(self):
        order_response = TestTenSellFilled.seller.postman.get_static_postman(TestTenSellFilled.seller_token). \
            order_service.create_order_sync(TestTenSellFilled.order_sell)
        assert order_response['error'] is None

        seller_balance = TestTenSellFilled.seller.postman.get_static_postman(TestTenSellFilled.seller_token). \
            balance_service.get_all_currencies_balance(TestTenSellFilled.seller.customer_id)
    ...
    def test_place_buy_order(self):
        order_response = TestTenSellFilled.buyer.postman.get_static_postman(TestTenSellFilled.buyer_token). \
            order_service.create_order_sync(TestTenSellFilled.order_buy)
        assert order_response['error'] is None
    ...
    def test_verify_buyer_balance(self):
        ...
        buyer_balance = TestTenSellFilled.buyer.postman.get_static_postman(TestTenSellFilled.buyer_token). \
            balance_service.get_all_currencies_balance(TestTenSellFilled.buyer.customer_id)
    ...
    def test_restore_original_price(self):
        Instruments.set_price_last_trade(instrument_id, TestTenSellFilled.original_last_trade_price)
        Instruments.set_ticker_last_price(instrument_id, TestTenSellFilled.original_ticker_last)
