...
@allure.feature("Sell Order is 10% filled - order parameters are updated")
@allure.story("Partially filled order parameters are updated - Sell.")
...
@allure.testcase(BaseConfig.GITLAB_URL + "/partially_filled_orders_tests/C6869_sell_order_params_10_filled_test.py",
                 "TestPartiallyFilledSell_10")
...
class TestOrderPartiallyFilled(object):
    ...
    def test_change_reference_price(self, r_customer):
        TestOrderPartiallyFilled.original_last_trade_price = Instruments.get_price_last_trade(instrument_id)
        ...
        TestOrderPartiallyFilled.original_ticker_last = Instruments.get_ticker_last_price(instrument_id)
    ...
    def test_place_order_sell(self, r_customer):
        ...
        order_response = r_customer.postman.order_service.create_order_sync(TestOrderPartiallyFilled.order_sell)
        assert order_response['error'] is None

        TestOrderPartiallyFilled.placed_sell_order_id = order_response['result']['orderId']

    @allure.step("Place the Buy order, quantity - 70.")
    @automation_logger(logger)
    def test_place_order_buy(self, r_customer):
        order_response = r_customer.postman.order_service.create_order_sync(TestOrderPartiallyFilled.order_buy)
        assert order_response['error'] is None

        TestOrderPartiallyFilled.placed_buy_order_id = order_response['result']['orderId']
    ...
    def test_restore_original_price(self):
        ...
        Instruments.set_price_last_trade(instrument_id, TestOrderPartiallyFilled.original_last_trade_price)
        Instruments.set_ticker_last_price(instrument_id, TestOrderPartiallyFilled.original_ticker_last)
    ...
    def test_find_orders_in_db(self, r_customer):
        ...
        buy_order_in_db = Order.orders_data_converter(
            Instruments.get_order_by_id(TestOrderPartiallyFilled.placed_buy_order_id))
        ...
        # "Sell" order, 10% filled
        sell_order_in_db = Order.orders_data_converter(
            Instruments.get_order_by_id(TestOrderPartiallyFilled.placed_sell_order_id))
    ...
    def test_find_orders_in_redis(self, r_customer):
        ...
        print(F"Order Control: {TestOrderPartiallyFilled.placed_sell_order_id}")

        # Verifying order details in Redis
        assert orders_verification['id'] == TestOrderPartiallyFilled.placed_sell_order_id
        ...
        assert float(quantity_sell * 0.1) == float(redis_filled_quantity)
