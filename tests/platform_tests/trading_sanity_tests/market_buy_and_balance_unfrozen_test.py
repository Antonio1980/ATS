...
@allure.title("TRADING SANITY TEST 'TS- 6'")
@allure.description("""
    Verify that customer balance unfrozen when market buy order placed.
    """)
@allure.testcase(BaseConfig.GITLAB_URL + "/balance_sanity_tests/market_buy_and_balance_unfrozen_test.py",
...
class TestMarketBuyUnfrozenAndBalanceUpdated(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.currency_id = 1
        self.customer.postman.balance_service.add_balance(self.customer.customer_id, self.currency_id, 100000000)
