...
@allure.description("""
    ...
    """)
@allure.testcase(BaseConfig.GITLAB_URL + "/balance_sanity_tests/new_sell_trade_added_to_trade_history_test.py",
                 "TestNewSellTradeIsAddedToTradeHistory")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.ui
@pytest.mark.trading_sanity
class TestNewSellTradeIsAddedToTradeHistory(unittest.TestCase):
    test_case = "TS- 9"
    ...
    def test_new_sell_trade_is_added_to_trade_history(self):
        ...
        try:
            ...
            price_sell = float(Instruments.get_orders_best_price_and_quantity(self.instrument_id, "sell", 1)[0])
            order_limit_sell = Order().set_order(2, self.instrument_id, quantity, price_sell)
            order_response_sell = self.customer.postman.order_service.create_order_sync(order_limit_sell)
            assert order_response_sell['error'] is None
            order_id_sell = order_response_sell['result']['orderId']

            time.sleep(10.0)
            trade_id = \
                Instruments.run_mysql_query("SELECT id FROM trades_crypto WHERE orderId = " + order_id_sell + ";")[0][0]
            ...
            price_from_ui_sell = (self.browser.wait_element_presented(self.driver,
                                                                      "//*[@id ='tradesHistoryPosition_" + str(
                                                                          trade_id) +
