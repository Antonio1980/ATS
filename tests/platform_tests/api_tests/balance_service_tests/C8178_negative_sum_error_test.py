...
# In this test we are verifying that there is no option to perform "freeze" and "unfreeze" operations with negative
# amount.


@allure.feature("Balance")
@allure.story("No option to freeze or unfreeze a negative sum.")
...
@allure.description("""
    ...
    2. Freeze all funds that aren't already frozen except one coin that remains available.
    3. Send negative parameter.
    """)
@allure.severity(allure.severity_level.BLOCKER)
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case, name='API BASE URL')
@allure.testcase(BaseConfig.GITLAB_URL + "/api_tests/balance_service_tests/C8178_negative_sum_error_test.py",
...
class TestUnfreezeAmountNegative(object):
    ...
    @allure.step("Freeze all funds that aren't already frozen except one coin that remains available.")
    @automation_logger(logger)
    ...
    @allure.step("Send negative parameter.")
    @automation_logger(logger)
    def test_negative_amount_rejceted(self, r_customer):
        # Freeze
        response = r_customer.postman.balance_service. \
            subtract_transaction_initialize(r_customer.customer_id, test_currency, -1)

        print(f"Freeze response {response}")
        logger.logger.info(f"Freeze response {response}")

        assert response['error']['code'] == expected_error
        assert response['error']['message'] == 'Given amount is negative'

        # Unfreeze
        response = r_customer.postman.balance_service.subtract_transaction_partial_rollback(
            TestUnfreezeAmountNegative.freeze_transaction_id, r_customer.customer_id, test_currency, -1)
