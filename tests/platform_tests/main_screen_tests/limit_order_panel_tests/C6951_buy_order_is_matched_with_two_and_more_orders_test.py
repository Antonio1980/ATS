...
@allure.description("""
    "Buy" Order is matched with two and more Orders, API
    1. Select an instrument(1022).
    2. Place a new "Buy" order with quantity that greater than the sum of the first 2 quantities in Order Book,
...
@allure.testcase(BaseConfig.GITLAB_URL +
                 "/main_screen_tests/limit_order_panel_tests/C6951_buy_order_is_matched_with_two_and_more_orders_test.py",
                 "TestBuyOrderIsMatchedWithTwoAndMoreOrders")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.functional
@pytest.mark.limit_order
@pytest.mark.limit_order_api
@pytest.mark.order_management
class TestBuyOrderIsMatchedWithTwoAndMoreOrders(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer.postman.balance_service.add_balance(self.customer.customer_id, 2, 150000)  # USD
        self.test_run = BaseConfig.TESTRAIL_RUN

    @allure.step("Starting with: test_buy_order_is_matched_with_two_and_more_orders")
    @automation_logger(logger)
    def test_buy_order_is_matched_with_two_and_more_orders(self):
        logger.logger.info("TEST CASE N: {0}".format(test_case))
        logger.logger.info(
            "method test_buy_order_is_matched_with_two_and_more_orders")
        result = 0
        try:
            ...
            quantity_m = Instruments.get_min_order_amount(self.instrument_id)
            self.customer.postman.balance_service.add_balance(self.customer.customer_id, 6, quantity_m + 6)
            order_limit_1 = Order().set_order(2, self.instrument_id, quantity_m + 5, price_1)
            ...
            self.customer.postman.balance_service.add_balance(self.customer.customer_id, 6, quantity_m + 20)
            order_limit_2 = Order().set_order(2, self.instrument_id, quantity_m + 16, price_2)
            ...
            self.customer.postman.balance_service.add_balance(self.customer.customer_id, 6, quantity_m + 20)
            order_limit_3 = Order().set_order(2, self.instrument_id, quantity_m + 16, price_3)
            ...
            order_id_query = ("SELECT id FROM orders WHERE customerId = " + str(
                self.customer.customer_id) + " and direction = 'buy' ORDER BY orders.dateInserted DESC limit 1;")
