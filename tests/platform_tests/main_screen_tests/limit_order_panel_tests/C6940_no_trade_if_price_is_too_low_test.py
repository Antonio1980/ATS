...
@allure.description("""
    No trade if the price is too low, API
    1. Select an instrument(1007).
    2. Find the best price offered(the lowest) from Order Book
    3. Place a "Sell" order. Set a price that is much higher then the "Best price" offered
    4. Verify that the order you placed wasn't matched
    """)
@allure.severity(allure.severity_level.CRITICAL)
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case,
             name='No trade if the price is too low')
@allure.testcase(BaseConfig.GITLAB_URL +
                 "/main_screen_tests/limit_order_panel_tests/C6940_no_trade_if_price_is_too_low_test.py",
                 "TestNoTradeIfPriceIsTooLow")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.functional
@pytest.mark.limit_order
@pytest.mark.limit_order_api
@pytest.mark.order_management
class TestNoTradeIfPriceIsTooLow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer.postman.balance_service.add_balance(int(self.customer.customer_id), 3, 100000)
        self.test_run = BaseConfig.TESTRAIL_RUN

    @allure.step("Starting with: test_no_trade_if_price_is_too_low")
    @automation_logger(logger)
    def test_no_trade_if_price_is_too_low(self):
        logger.logger.info("TEST CASE N: {0}".format(test_case))
        logger.logger.info("method test_no_trade_if_price_is_too_low ")
        result = 0
        try:
            time.sleep(3.0)
            ...
            best_price_and_quantity = Instruments.get_orders_best_price_and_quantity(self.instrument_id, "sell", 1)
