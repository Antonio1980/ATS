...
@allure.description("""
    api test.
    Verify that the values in the DB are identical to those set after placing Buy order , API
    1. Place Buy order
...
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case,
             name='Placing "Limit" type order - "Buy"')
@allure.testcase(
    BaseConfig.GITLAB_URL + "/main_screen_tests/limit_order_panel_tests/C6920_placing_limit_type_order_buy_test.py",
    "TestPlacingLimitTypeOrderBuy")
...
class PlacingLimitTypeOrderSellTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer.postman.balance_service.add_balance(self.customer.customer_id, 50, 100000)  # BTC crypto
        self.customer.postman.balance_service.add_balance(self.customer.customer_id, 1, 100000)
    ...
    @automation_logger(logger)
    def test_placing_limit_type_order_buy(self):
