...
@allure.description("""
    ...
    UI test.
    "Sell" Order Is Partially Filled - UI Verification
    1. Open WTP
    2. Select an instrument(1022).
    3. Place a new "Sell" order with price between the first and the second price from Order Book .
    4. Verify that Order is presented in UI
    """)
@allure.severity(allure.severity_level.CRITICAL)
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case,
             name='"Sell" Order Is Partially Filled - UI Verification')
@allure.testcase(BaseConfig.GITLAB_URL + "/main_screen_tests/limit_order_panel_tests/C6944_sell_order_is_partially_filled_ui_verification_test.py",
                "TestSellOrderIsPartiallyFilledUiVerificationTest")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.ui
@pytest.mark.limit_order
@pytest.mark.order_management
@ddt
class TestSellOrderIsPartiallyFilledUiVerificationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer.postman.balance_service.add_balance(self.customer.customer_id, 6, 10000)
        self.test_run = BaseConfig.TESTRAIL_RUN
        self.limit_order_panel = LimitOrderPanelPage()
        self.locators = self.limit_order_panel.locators
        self.browser = self.customer.get_browser_functionality()

    @allure.step("Starting with: test_sell_order_is_partially_filled_ui_verification")
    @data(*Instruments.get_csv_data(BaseConfig.BROWSERS))
    @unpack
    @automation_logger(logger)
    def test_sell_order_is_partially_filled_ui_verification(self, browser):
        logger.logger.info("TEST CASE N: {0}".format(test_case))
        logger.logger.info("method test_sell_order_is_partially_filled_ui_verification with: browser- {0} ".format(browser))
        ...
        try:
            ...
            assert self.browser.wait_element_presented(self.driver, self.locators.USD, delay+5.0)
            ...
            order_limit_1 = Order().set_order(1, self.instrument_id, quantity + 16, price_1)
            ...
            price_2 = round((price_1 + (price_1 * 0.3)), price_tail_digits)
            self.customer.postman.balance_service.add_balance(self.customer.customer_id, 6, quantity + 20)
            order_limit_2 = Order().set_order(1, self.instrument_id, quantity + 5, price_2)
            ...
            order_buy = Order().set_order(2, self.instrument_id, quantity, price)
            ...
            data_query = ("SELECT id, price FROM orders WHERE customerId = " + str(self.customer.customer_id) +
                          " and direction = 'sell' ORDER BY orders.dateInserted DESC limit 1;")
            ...
            filled_quantity_from_ui = self.browser.find_element(self.driver,
            ...
            assert float(price_from_ui) == price
            assert float(filled_quantity_from_ui[:-1]) == filled_quantity
