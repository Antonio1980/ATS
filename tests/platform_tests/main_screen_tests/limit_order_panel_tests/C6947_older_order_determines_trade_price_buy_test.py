...
@allure.description("""
    API test.
    Older order determines the trade price - "Buy"
    1. Select an instrument(1022).
    2. Place a new "Buy" order with price between the first and the second price from Order Book and quantity
...
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case,
             name='Older order determines the trade price - "Buy"')
@allure.testcase(BaseConfig.GITLAB_URL +
                 "/main_screen_tests/limit_order_panel_tests/C6947_older_order_determines_trade_price_buy_test.py",
                 "TestOlderOrderDeterminesTradePriceBuy")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.functional
@pytest.mark.limit_order
@pytest.mark.limit_order_api
@pytest.mark.order_management
class TestOlderOrderDeterminesTradePriceBuy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer.postman.balance_service.add_balance(int(self.customer.customer_id), 2, 50000.0)  # EUR
        self.test_run = BaseConfig.TESTRAIL_RUN

    @allure.step("Starting with: test_older_order_determines_trade_price_buy")
    @automation_logger(logger)
    def test_older_order_determines_trade_price_buy(self):
        result = 0
        try:
            price_1 = Instruments.get_price_last_trade(self.instrument_id)
            quantity = Instruments.get_min_order_amount(self.instrument_id)
            self.customer.postman.balance_service.add_balance(self.customer.customer_id, 6, 100000.0)
            ...
            order_id = str(Instruments.run_mysql_query("SELECT id FROM orders WHERE customerId = " + str(
                self.customer.customer_id) + " and direction = 'buy'ORDER BY orders.dateInserted DESC limit 1;")[0][0])
            ...
            price_from_order = float(Instruments.run_mysql_query(
                ...
                " and direction = 'buy' GROUP BY price; ")[0][0])
