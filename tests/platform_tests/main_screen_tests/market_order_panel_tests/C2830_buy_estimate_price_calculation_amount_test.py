...
@allure.description("""
    UI test.
    Verified that Estimate Price is calculated per amount in "Buy" section, UI
    1. Open WTP
    2. Select an instrument(1032).
    3. Enter a new "base" currency amount ("Buy" section). It must be smaller than the quantity offered for sale in
       the first order from Order Book.
    4. Check that the "Estimate Price" presented is equals to the price of the first order from Order Book
    5. Enter a new "base" currency amount ("Buy" section). It must be equal to a sum of the first and half 
       of the second orders quantities from Order Book.
    6. Check that the "Estimate Price" presented is equals to the average price of the first two orders from 
       Order Book calculated according to their fill rate.
    7. Enter a new "base" currency amount ("Buy" section). It must be equal to a sum of the first and the second
...
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case,
             name='"Buy" section - Estimate Price is calculated per amount')
@allure.testcase(BaseConfig.GITLAB_URL +
                 "/main_screen_tests/market_order_panel_tests/C2830_buy_estimate_price_calculation_amount_test.py",
                 "TestBuyEstimatePriceCalculationAmount")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.ui
@pytest.mark.market_order
@pytest.mark.order_management
@ddt
class TestBuyEstimatePriceCalculationAmount(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer.postman.balance_service.add_balance(int(self.customer.customer_id), 10, 10000)  # JPY currency
        self.test_run = BaseConfig.TESTRAIL_RUN
        self.market_order_panel = MarketOrderPanelPage()
        self.locators = self.market_order_panel.locators
        self.browser = self.customer.get_browser_functionality()

    @allure.step("Starting with: test_buy_estimate_price_calculation_amount")
    @data(*Instruments.get_csv_data(BaseConfig.BROWSERS))
    @unpack
    @automation_logger(logger)
    def test_buy_estimate_price_calculation_amount(self, browser):
        ...
        try:
            ...
            assert self.browser.wait_element_presented(self.driver,
                                                       ...
                                                       delay + 5)
            best_price_and_quantity = Instruments.get_orders_best_price_and_quantity(1032, "buy", 2)[:50]
            ...
            enter_amount_buy = self.browser.wait_element_presented(self.driver, self.locators.AMOUNT_FOR_BUY, delay)
            order_quantity_one = str(round(quantity - (quantity / 100 * 20), 5))
            self.browser.send_keys(enter_amount_buy, order_quantity_one)
            expected_price_one = str(int(price))
            time.sleep(2)
            estimated_price_buy_one = self.browser.execute_js(self.driver, self.locators.ESTIMATED_PRICE_FOR_BUY_JQ)
            assert expected_price_one == estimated_price_buy_one
            order_quantity_two = (quantity + (second_quantity / 2))
            order_quantity_two_str = str(order_quantity_two)
            self.browser.send_keys(enter_amount_buy, order_quantity_two_str)
            time.sleep(2)
            estimated_price_buy_two = self.browser.execute_js(self.driver, self.locators.ESTIMATED_PRICE_FOR_BUY_JQ)
            expected_price_two = str(
                int(((price * quantity) + (second_price * second_quantity * 0.5)) / order_quantity_two))
            assert estimated_price_buy_two == expected_price_two
            order_quantity_three = quantity + second_quantity
            order_quantity_three_str = str(order_quantity_three)
            self.browser.send_keys(enter_amount_buy, order_quantity_three_str)
            time.sleep(2)
            estimated_price_buy_three = self.browser.execute_js(self.driver, self.locators.ESTIMATED_PRICE_FOR_BUY_JQ)
            expected_price_three = str(
                int(((price * quantity) + (second_price * second_quantity)) / order_quantity_three))
            assert estimated_price_buy_three == expected_price_three
