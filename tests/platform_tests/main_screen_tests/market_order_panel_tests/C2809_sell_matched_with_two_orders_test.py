...
@allure.description("""
    Functional test. API
    "Sell" order is matched with two and more orders. 
    1. Select an instrument(1022).
    2. Place a new "Sell" order with amount greater than the sum of the first orders from order book.
...
@allure.testcase(BaseConfig.GITLAB_URL + "/main_screen_tests/market_order_panel_tests/C2809_sell_matched_with_two_orders_test.py",
                 "TestSellMatchedWithTwoOrders")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.functional
@pytest.mark.market_order
@pytest.mark.order_management
class TestSellMatchedWithTwoOrders(unittest.TestCase):
    @allure.step("SetUp:  calling registered customer and increase him balance."
                 "Define ID of instrument for 'Buy' order ")
    @automation_logger(logger)
    def setUp(self):
        self.customer = RegisteredCustomer()
        self.customer.postman.balance_service.add_balance(self.customer.customer_id, 8, 20500)
        self.test_run = BaseConfig.TESTRAIL_RUN
        self.instrument = 1022

    @allure.step("Starting with: test_sell_matched_with_two_orders")
    @automation_logger(logger)
    def test_sell_matched_with_two_orders(self):
        result = 0
        try:
            best_price_and_quantity = Instruments.get_orders_best_price_and_quantity(self.instrument, "sell", 2)[:50]
            ...
            order_market_sell = Order().set_order(2, self.instrument, order_quantity)
            ...
            order_id = str(Instruments.run_mysql_query(
                ...
                " and direction = 'sell' ORDER BY orders.dateInserted DESC limit 1;")[0][0])
            ...
            sum_quantity_for_first_best_price = float(
                Instruments.run_mysql_query("SELECT SUM(quantity) FROM trades_crypto WHERE price = "
                                            ...
                                            + order_id + " and direction = 'sell';")[0][0])
