...
@allure.description("""
    UI test.
    Verified that Estimate Price is calculated per Order Book in "Buy" section, UI
    1. Open WTP
    2. Select an instrument(1032).
    3. Enter a new "base" currency amount ("Buy" section). It's "amount" must be equal to the sum of amounts of 
       all from Order Book.
...
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case,
             name='"Buy" section - Estimate Price is calculated per Order Book')
@allure.testcase(BaseConfig.GITLAB_URL +
                 "/main_screen_tests/market_order_panel_tests/C2832_buy_estimate_price_calculation_order_book_test.py",
                 "TestBuyEstimatePriceCalculationOrderBook")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.ui
@pytest.mark.market_order
@pytest.mark.order_management
@ddt
class TestBuyEstimatePriceCalculationOrderBook(unittest.TestCase):
    ...
    @allure.step("Starting with: test_buy_estimate_price_calculation_order_book")
    @data(*Instruments.get_csv_data(BaseConfig.BROWSERS))
    @unpack
    @automation_logger(logger)
    def test_buy_estimate_price_calculation_order_book(self, browser):
