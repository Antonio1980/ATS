...
@allure.description("""
    Functional test. API 
    "Sell" order is filled or terminated. 
    1. Select an instrument(1022).
    2. Place a new "Sell" order with amount greater than the sum of amounts of all orders from order book.
...
@allure.link(BaseConfig.TEST_RAIL_URL + "/index.php?/cases/view/" + test_case,
             name='"Sell" order is filled or terminated')
@allure.testcase(BaseConfig.GITLAB_URL + "/main_screen_tests/market_order_panel_tests/C2815_sell_order_filled_or_terminated_test.py",
                 "TestSellOrderFilledOrTerminated")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.functional
@pytest.mark.market_order
@pytest.mark.order_management
class TestSellOrderFilledOrTerminated(unittest.TestCase):
    @allure.step("SetUp:  calling registered customer and increase him balance."
                 "Define ID of instrument for 'Buy' order ")
    @automation_logger(logger)
    def setUp(self):
        self.customer = RegisteredCustomer()
        self.customer.postman.balance_service.add_balance(self.customer.customer_id, 8, 800000)
        self.test_run = BaseConfig.TESTRAIL_RUN
        self.id_instrument = 1022

    @allure.step("Starting with: test_sell_order_filled_or_terminated")
    @automation_logger(logger)
    def test_sell_order_filled_or_terminated(self):
        result = 0
        try:
            best_price_and_quantity = Instruments.get_orders_best_price_and_quantity(self.id_instrument, "sell", 2)
            ...
            order_market_sell = Order().set_order(2, self.id_instrument, amount_for_trade)
