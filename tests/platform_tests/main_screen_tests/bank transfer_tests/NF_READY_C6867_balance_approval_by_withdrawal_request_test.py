...
class VerifyBalanceByApprovedCRMWithdrawalRequestTest(unittest.TestCase):
    ...
    def test_verify_balance_by_approved_crm_withdrawal_request(self, browser):
        ...
        try:
            ...
            time.sleep(5)
            eur_currency_avail_balance_before =\
                self.browser.execute_js(self.driver, self.locators.EUR_CURRENCY_AVAILABLE_BALANS_FUNDS_PAGE_JQ)
            step3 = True if eur_currency_avail_balance_before != "-" else False
            withdrawal_button = self.browser.wait_element_presented(self.driver, self.locators.WITHDRAWAL_BUTTON_EUR,
                                                                    delay)
            self.browser.click_on_element(withdrawal_button)
            time.sleep(3)
            ...
            input_confirmation_code = self.browser.wait_element_presented(self.driver,
                                                                          self.locators.ENTER_CONFIRMATION_CODE_FOR_EUR,
                                                                          delay)
