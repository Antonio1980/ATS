...
@allure.description("""
    Functional test. UI.
    Validation of 'frozen' balance after matched "Buy" Limit order via UI.
    1. Checking of 'frozen' balance before orders.
    2. Place 'Buy' Limit order.      
    4. Checking of'frozen' balances after orders. 
    Calculation: frozen_balance_after = "€0.00"
    """)
@allure.severity(allure.severity_level.CRITICAL)
@allure.link(BaseConfig.TEST_RAIL_URL + '/index.php?/cases/view/' + test_case,
             name='Market Buy Order Matched - Balance Unfrozen')
@allure.testcase(BaseConfig.GITLAB_URL + "/balance_tests/C7321_market_buy_order_matched_balance_unfrozen_test.py",
                 "TestMarketBuyOrderMatchedBalanceUnfrozen")
@pytest.mark.usefixtures("r_time_count")
@pytest.mark.balance
@ddt
class TestMarketBuyOrderMatchedBalanceUnfrozen(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer.postman.balance_service.add_balance(int(self.customer.customer_id), 2, 2000)
        self.market_order_panel = MarketOrderPanelPage()
        self.locators = self.market_order_panel.locators
        self.browser = self.customer.get_browser_functionality()
        self.instrument_id = 1013  # ETH/EUR

    @allure.step("Starting with: test_market_buy_order_matched_balance_unfrozen")
    @automation_logger(logger)
    @data(*Instruments.get_csv_data(BaseConfig.BROWSERS))
    @unpack
    def test_market_buy_order_matched_balance_unfrozen(self, browser):
        logger.logger.info("TEST CASE N: {0}".format(test_case))
        logger.logger.info(
            "method test_market_buy_order_matched_balance_unfrozen,"
