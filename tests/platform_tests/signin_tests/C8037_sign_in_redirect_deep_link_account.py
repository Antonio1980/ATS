...
@allure.description("""
    Functional test.
    Verification of SignIn by deep link to myAccount ,  UI test
    1. Go by link.
...
@allure.testcase(BaseConfig.GITLAB_URL + "/signin_tests/C8037_sign_in_redirect_deep_link_account.py",
                 "TestSignInRedirectDeepLinkMyAccount")
@pytest.mark.smoke
@pytest.mark.ui
@pytest.mark.incremental
@pytest.mark.usefixtures('r_time_count', 'r_customer', )
class TestSignInRedirectDeepLinkMyAccount(object):
    driver = WebDriverFactory.get_driver(Browsers.CHROME.value)
    delay = float(BaseConfig.UI_DELAY)
    browser = Browser()
    sign_in_page = SignInPage()
    url = BaseConfig.WTP_BASE_URL + '/appProxy/exchange.html?activeZone=myAccount'
    logger.logger.info("TEST CASE N: {0}".format(test_case))
    logger.logger.info(
        "method sign_in_redirect_deep_link_my_account")

    @automation_logger(logger)
    def test_open_link(self):
        self.browser.go_to_url(self.driver, self.url)
        session_expired = self.browser.wait_element_presented(self.driver,
                                                              self.sign_in_page.locators.SESSION_EXPIRED_TITLE,
                                                              self.delay + 5)
