...
class ChangePasswordNewUserTest(unittest.TestCase):
    ...
    def test_login_with_new_password(self):
        ...
        result1, result2, result3, result4, result5, result6, result7 = False, False, False, False, False, False, False
        try:
            ...
            result4 = self.login_page.go_to_login_page(self.driver, user_index_page_url)
            result5 = self.login_page.login(self.driver, self.username, new_password)
            result6 = self.login_page.set_new_password(self.driver, new_password, new_password + "Qa")
            result7 = self.home_page.logout(self.driver, delay)
        finally:
            if result1 and result2 and result3 and result4 and result5 and result6 and result7 is True:
