...
class LinksOnVerifyEmailScreenTest(unittest.TestCase):
    ...
    def test_links_on_verify_email_screen(self):
        ...
        try:
            ...
            result3 = self.open_account_page.click_on_link_on_email_screen(self.driver, self.home_page.wtp_open_account_url, 1)
            #res1 = self.home_page.go_back_and_wait(self.driver, wtp_open_account_url, delay)
            result4 = self.open_account_page.click_on_link_on_email_screen(self.driver, wtp_dashboard_url, 2)
            res2 = self.home_page.go_back_and_wait(self.driver, self.home_page.wtp_open_account_url, delay)
            result5 = self.open_account_page.click_on_link_on_email_screen(self.driver, self.home_page.wtp_open_account_url, 3)
            time.sleep(15)
            # Opens email box, clicks on "Very email" button and checks that redirected to OpenAccountPage url.
            result6 = self.open_account_page.get_email_updates(self.driver, self.open_account_page.email, 3)
        finally:
