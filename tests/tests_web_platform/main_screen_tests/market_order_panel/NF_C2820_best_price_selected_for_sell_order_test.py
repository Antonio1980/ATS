...
#need to run precondition "create_customer_pending"
@test(groups=['market_order_panel', ])
class BestPriceSelectedForSellOrders(unittest.TestCase):
    def setUp(self):
        self.test_case = '2820'
    ...
    def test_best_price_selected_for_sell_orders(self):
        step1 = False
        try:
            best_price_and_quantity = Instruments.get_best_price_and_quantity_for_order_sell("1013") # "1013"  - BTC/USD Instrument
            ...
            order_response = postman_client.create_order("2", order_quantity, "1013")
            query_id_trade_crypto = "SELECT id FROM trades_crypto WHERE customerId = " + self.customer_id + " and direction = 'sell';"
