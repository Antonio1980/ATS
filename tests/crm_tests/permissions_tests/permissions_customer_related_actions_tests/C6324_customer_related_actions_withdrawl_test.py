...
@ddt
class TestCustomerRelatedActionsWithdrawals(unittest.TestCase):
    def setUp(self):
        self.test_case = '6324'
        ...
        self.local_permissions_entity_name = "'Customer Related Actions - Withdrawal'"
        self.local_sub_module_name = "'Customer Related Actions - Withdrawal'"
    ...
    def test_customer_permission_test(self, browser):
        ...
        delay = 5
        step1, step2, step3, step4, step5, step6, step7, step8, step9, step10 = False, False, False, False, False, False, False, False, False, False
        try:
            ...
            customer_id_withdrawals_list = Browser.find_elements(self.driver,
                                                       self.locators.CUSTOMER_ID_LIST_ON_WITHDRAWALS_PAGE)
            first_customer_id_withdrawals_link = customer_id_withdrawals_list[0]
            assert Browser.wait_element_presented(self.driver, first_customer_id_withdrawals_link, delay)
            Browser.click_on_element(first_customer_id_withdrawals_link)
            assert Browser.wait_element_presented(self.driver,
                                                  self.locators.DEPOSITS_WITHDRAWLS_TITLE_TAB_ON_CUSTOMER_PAGE,
                                                  delay)
            assert Browser.wait_element_presented(self.driver,
                                                  self.locators.WITHDRAWALS_TITLE_ON_WITHDRWALS_BLOCK,
                                                  delay)
            id_deposit = Browser.find_elements(self.driver,
                                               self.locators.NOT_EDIT_ID_WITHDRAWALS_ON_CUSTOMER_PAGE)
            first_id_deposit = id_deposit[0]
            assert Browser.wait_element_presented(self.driver, first_id_deposit, delay)
            assert Browser.check_element_not_presented(self.driver,
                                                       self.locators.EDIT_ID_WITHDRAWALS_LINK_ON_CUSTOMER_PAGE, delay)
            assert Browser.check_element_not_presented(self.driver, self.locators.ADD_NEW_WITHDRAWAL_BUTTON, delay)
            step8 = True
            ...
            assert Browser.check_element_not_presented(self.driver, self.locators.ADD_NEW_WITHDRAWAL_BUTTON, delay)
            edit_withdrawals_id_list = Browser.find_elements(self.driver,
                                                         self.locators.EDIT_ID_WITHDRAWALS_LINK_ON_CUSTOMER_PAGE)
            first_withdrwal_id_link = edit_withdrawals_id_list[0]
            Browser.click_on_element(first_withdrwal_id_link)
            assert Browser.wait_element_presented(self.driver, self.locators.EDIT_WITHDRAWAL_DETAILS_TITLE_ON_MODAL_WIN,
                                                  delay)
            close_button_for_modal_edit_withdrawal = Browser.wait_element_presented(self.driver,
                                                                                 self.locators.CLOSE_BUTTON_ON_MODAL_FOR_EDIT_WITHDRAWAL,
                                                                                 delay)
            Browser.click_on_element(close_button_for_modal_edit_withdrawal)
            step9 = True
            # update method with  2  : hasView = 1, hasEdit = 1, hasCreate = 1
            self.home_page.update_view_edit_create_permissions(self.local_permissions_entity_name,
                                                               self.permissions_group_id, 2)
            Browser.refresh_page(self.driver)
            assert Browser.wait_element_presented(self.driver,  first_withdrwal_id_link, delay)
            add_new_withdrawal_button = Browser.wait_element_presented(self.driver, self.locators.ADD_NEW_WITHDRAWAL_BUTTON,
                                                                    delay)
            Browser.click_on_element(add_new_withdrawal_button)
            assert Browser.wait_element_presented(self.driver, self.locators.ADD_NEW_WITHDRAWAL_REQUEST_TITLE_ON_MODAL_WIN,
                                                  delay)
            step10 = True
        finally:
