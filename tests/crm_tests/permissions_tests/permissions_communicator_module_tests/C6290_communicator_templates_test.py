...
@ddt
class TestCommunicatorTemplates(unittest.TestCase):
    def setUp(self):
        self.test_case = '6290'
        ...
        self.local_permissions_entity_name = "'Communicator - Templates'"
        self.local_sub_module_name = "'Communicator - Templates'"
    ...
    def test_communicator_permission_test(self, browser):
        ...
        try:
            ...
            communicator_dropdown_button = Browser.find_element_by(self.driver, self.locators.COMMUNICATOR_MENU_ID, "id")
            Browser.click_on_element(communicator_dropdown_button)
            templates_dropdown_title = Browser.wait_element_presented(self.driver, self.locators.TEMPLATES_DROPDOWN_TITLE, delay)
            Browser.click_on_element(templates_dropdown_title)
            assert Browser.wait_element_presented(self.driver, self.locators.NOT_EDIT_TEMPLATES, delay)
            assert Browser.check_element_not_presented(self.driver, self.locators.EDIT_TEMPLATES_LINK, delay)
            assert Browser.check_element_not_presented(self.driver, self.locators.ADD_TEMPLATE_BUTTON, delay)
            step5 = True
            ...
            assert Browser.check_element_not_presented(self.driver, self.locators.ADD_TEMPLATE_BUTTON, delay)
            edit_template_link = Browser.wait_element_presented(self.driver, self.locators.EDIT_TEMPLATES_LINK, delay)
            Browser.click_on_element(edit_template_link)
            assert Browser.wait_element_presented(self.driver, self.locators.EDIT_TITLE_ON_EDIT_PAGE_TEMPLATES, delay)
            ...
            self.home_page.update_view_edit_create_permissions(self.local_permissions_entity_name,
                                                               self.permissions_group_id, 2)
            Browser.refresh_page(self.driver)
            assert Browser.wait_element_presented(self.driver, self.locators.EDIT_TEMPLATES_LINK, delay)
            add_campaign_button = Browser.wait_element_presented(self.driver, self.locators.ADD_TEMPLATE_BUTTON, delay)
