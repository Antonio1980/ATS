...
@ddt
class TestCommunicatorScheduledMessaging(unittest.TestCase):
    def setUp(self):
        self.test_case = '6289'
        ...
        self.local_permissions_entity_name = "'Communicator - Scheduled Messaging'"
        self.local_sub_module_name = "'Communicator - Scheduled Messaging'"
    ...
    def test_communicator_permission_test(self, browser):
        ...
        try:
            ...
            scheduled_messages_dropdown_title = Browser.wait_element_presented(self.driver,
                                                                               self.locators.SCHEDULED_MESSAGING_DROPDOWN_TITLE,
                                                                               delay)
            Browser.click_on_element(scheduled_messages_dropdown_title)
            assert Browser.check_element_not_presented(self.driver, self.locators.EDIT_SCHEDULED_MESSAGE_LINK, delay)
            assert Browser.check_element_not_presented(self.driver, self.locators.ADD_NEW_SCHEDULED_MESSAGE, delay)
            step5 = True
            ...
            assert Browser.check_element_not_presented(self.driver, self.locators.ADD_NEW_SCHEDULED_MESSAGE, delay)
            edit_scheduled_message_link = Browser.wait_element_presented(self.driver,
                                                                         self.locators.EDIT_SCHEDULED_MESSAGE_LINK,
                                                                         delay)
            Browser.click_on_element(edit_scheduled_message_link)
            assert Browser.wait_element_presented(self.driver, self.locators.EDIT_TITLE_ON_EDIT_PAGE_SHEDULED_MESSAGE,
                                                  delay)
            ...
            self.home_page.update_view_edit_create_permissions(self.local_permissions_entity_name,
                                                               self.permissions_group_id, 2)
            Browser.refresh_page(self.driver)
            assert Browser.wait_element_presented(self.driver, self.locators.EDIT_SCHEDULED_MESSAGE_LINK, delay)
            add_scheduled_message = Browser.wait_element_presented(self.driver, self.locators.ADD_NEW_SCHEDULED_MESSAGE,
                                                                   delay)
            Browser.click_on_element(add_scheduled_message)
            assert Browser.wait_element_presented(self.driver,
                                                  self.locators.CREATE_NEW_SCHEDULED_MEASSAGE_TITLE_ON_SCHEDULED_MESSAGING_PAGE,
                                                  delay)
            step7 = True
        finally:
