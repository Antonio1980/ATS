...
def test_performance_for_order_service():
    order_buy = Order().set_order(1, instrument_id, min_order_quantity_for_instrument, price_buy)
    order_sell = Order().set_order(2, instrument_id, min_order_quantity_for_instrument, price_sell)
    ...
    while time.perf_counter() < time_out:
        ...
        try:
            l_customer.postman.order_service.create_order_sync(order_buy)
            l_customer.postman.order_service.create_order_sync(order_sell)
