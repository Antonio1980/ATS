...
def linux_distribution(full_distribution_name=True):
    ...
    * ``codename``:  The result of :func:`distro.codename`.
    ...
    The tests_data it returns may not exactly be the same, because it uses more tests_data
    sources than the original function, and that may lead to different tests_data if
    the Linux distribution is not consistent across multiple tests_data sources it
...
def info(pretty=False, best=False):
    ...
    information items are available in the underlying tests_data sources. The values
...
def os_release_info():
    ...
    from the os-release file tests_data source of the current Linux distribution.
...
def lsb_release_info():
    """
    Return a dictionary containing key-value pairs for the information items
    from the lsb_release command tests_data source of the current Linux distribution.
...
def distro_release_info():
    """
    Return a dictionary containing key-value pairs for the information items
    from the distro release file tests_data source of the current Linux distribution.
...
def os_release_attr(attribute):
    """
    Return a single named information item from the os-release file tests_data source
...
def lsb_release_attr(attribute):
    ...
    tests_data source of the current Linux distribution.
...
class LinuxDistribution(object):
    ...
    returns tests_data about the current Linux distribution (i.e. the distro this
    package runs on).

    Normally, it is not necessary to create additional instances of this class.
    However, in situations where control is needed over the exact tests_data sources
    ...
    def __init__(self,
        ...
        available tests_data sources, and stores that in private instance attributes.
        Subsequent access to the information items uses these private instance
        attributes, so that the tests_data sources are read only once.

        Parameters:

        * ``include_lsb`` (bool): Controls whether the
          `lsb_release command output`_ is included as a tests_data source.

          If the lsb_release command is not available in the program execution
          path, the tests_data source for the lsb_release command will be empty.

        * ``os_release_file`` (string): The path name of the
          `os-release file`_ that is to be used as a tests_data source.

          An empty string (the default) will cause the default path name to
          be used (see `os-release file`_ for details).

          If the specified or defaulted os-release file does not exist, the
          tests_data source for the os-release file will be empty.

        * ``distro_release_file`` (string): The path name of the
          `distro release file`_ that is to be used as a tests_data source.

          An empty string (the default) will cause a default search algorithm
          to be used (see `distro release file`_ for details).

          If the specified distro release file does not exist, or if no default
          distro release file can be found, the tests_data source for the distro
          release file will be empty.

        Public instance attributes:

        * ``os_release_file`` (string): The path name of the
          `os-release file`_ that is actually used as a tests_data source. The
          empty string if no distro release file is used as a tests_data source.

        * ``distro_release_file`` (string): The path name of the
          `distro release file`_ that is actually used as a tests_data source. The
          empty string if no distro release file is used as a tests_data source.
        ...
        * :py:exc:`UnicodeError`: A tests_data source has unexpected characters or
    ...
    def os_release_info(self):
        ...
        items from the os-release file tests_data source of the Linux distribution.
    ...
    def lsb_release_info(self):
        """
        Return a dictionary containing key-value pairs for the information
        items from the lsb_release command tests_data source of the Linux
        distribution.
    ...
    def distro_release_info(self):
        """
        Return a dictionary containing key-value pairs for the information
        items from the distro release file tests_data source of the Linux
        distribution.
    ...
    def os_release_attr(self, attribute):
        """
        Return a single named information item from the os-release file tests_data
    ...
    def lsb_release_attr(self, attribute):
        ...
        output tests_data source of the Linux distribution.
    ...
    def distro_release_attr(self, attribute):
        """
        Return a single named information item from the distro release file
        tests_data source of the Linux distribution.
