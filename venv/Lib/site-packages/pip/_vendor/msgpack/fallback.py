...
class Unpacker(object):
    ...
    :param file_like:
        ...
        If specified, unpacker reads serialized tests_data from it and :meth:`feed()` is not usable.
    ...
    :param int max_buffer_size:
        Limits size of tests_data waiting unpacked.  0 means system's INT_MAX (default).
        Raises `BufferFull` exception when it is insufficient.
        You should set this parameter when unpacking tests_data from untrusted source.
...
class Packer(object):
    ...
    def pack_ext_type(self, typecode, data):
        ...
        if not isinstance(data, bytes):
            raise TypeError("tests_data must have bytes type")
        L = len(data)
        if L > 0xffffffff:
            raise PackValueError("Too large tests_data")
        if L == 1:
            self._buffer.write(b'\xd4')
        elif L == 2:
            self._buffer.write(b'\xd5')
