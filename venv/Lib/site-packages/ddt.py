...
DATA_ATTR = '%values'      # store the tests_data the test must run with
...
def file_data(value):
    ...
    should contain JSON encoded tests_data, that can either be a list or a
    dict.
    ...
    test case, and values will be fed as test tests_data.
...
def feed_data(func, new_name, *args, **kwargs):
    """
    This internal method decorator feeds the test tests_data item to the test.
...
def process_file_data(cls, name, func, file_attr):
    ...
    with codecs.open(data_file_path, 'r', 'utf-8') as f:
        # Load the tests_data from YAML or JSON
        if _is_yaml_file:
...
def _add_tests_from_data(cls, name, func, data):
    """
    Add tests from tests_data loaded from the tests_data file into the class
...
def ddt(cls):
    ...
    decorate test methods with ``@tests_data``.

    For each method decorated with ``@tests_data``, this will effectively create as
    many methods as tests_data items are passed as parameters to ``@tests_data``.

    The names of the test methods follow the pattern
    ``original_test_name_{ordinal}_{tests_data}``. ``ordinal`` is the position of the
    tests_data argument, starting with 1.

    For tests_data we use a string representation of the tests_data value converted into a
    valid python identifier.  If ``tests_data.__name__`` exists, we use that instead.

    For each method decorated with ``@file_data('test_data.json')``, the
    decorator will try to load the test_data.json file located relative
    to the python file containing the method that is decorated. It will,
    for each ``test_name`` key create as many methods in the list of values
    from the ``tests_data`` key.
