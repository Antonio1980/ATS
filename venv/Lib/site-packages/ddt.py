...
# by the `ddt` class testrail_update.
...
def unpack(func):
    """
    Method testrail_update to add unpack feature.
...
def data(*values):
    """
    Method testrail_update to add to your test methods.
...
def feed_data(func, new_name, *args, **kwargs):
    """
    This internal method testrail_update feeds the test data item to the test.
...
def process_file_data(cls, name, func, file_attr):
    """
    Process the parameter in the `file_data` testrail_update.
...
def ddt(cls):
    """
    Class testrail_update for subclasses of ``unittest.TestCase``.

    Apply this testrail_update to the test case class, and then
    ...
    testrail_update will try to load the test_data.json file located relative
