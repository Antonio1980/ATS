...
options = [
    ...
    (("-D", "--define"),
     dict(dest="userdata_defines", type=parse_user_define, action="append",
          metavar="NAME=VALUE",
          help="""Define user-specific tests_data for the config.userdata dictionary.
...
class Configuration(object):
    ...
    def update_userdata(self, data):
        """Update userdata with tests_data and reapply userdata defines (cmdline).
        :param data:  Provides (partial) userdata (as dict)
        """
        self.userdata.update(data)
        if self.userdata_defines:
            # -- REAPPLY: Cmd-line defines (override configuration file tests_data).
            self.userdata.update(self.userdata_defines)
